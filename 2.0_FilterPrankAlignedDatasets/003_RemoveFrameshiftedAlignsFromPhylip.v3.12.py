...
fileextension = '.AA.Align.red.prank.fas-gb'
refname = 'Homo' # name of reference sequence for all alignments. We used human.
cutoff = 0.3 # pairwise deletion distance frequency cutoff value to mask all nucleotides in window when pairwise distance is greater than pairwise distance cutoff
...
phybindir = 'GblocksFiltered_AA_Alignments' # directory should be in current directory
outdir = 'GblocksAAFastaRemFrameshiftAlignments' # output directory to be made in current working directory
...
def main():
	...
	for f in filelist: # f is complete path to file with filename
		...
		i = 0 # beginning of alignment
		start = {}
		end = {}
		if seqs != 'empty':
			pdist, xlist = CalculatePairwiseDeletionDistanceForAlignedSeqsComparedToRefSeq(seqs, refname)
			...
			start, end = RecordValueIfGreaterThanTwoStandardDevsAwayFromMean(median, stddev, Zscore, cutoff, redpdist, i, lenseqs, start, end) # we use median instead of mean because less influence from outliers
			#start, end = RecordValueIfGreaterThanCutoffValue(pdist, cutoff, i, windsize, start, end)
			newseqs = MaskAlignedSeqsUsingStartAndEndCoordinates(seqs, start, end, missingchar, lenseqs, lenseqs)
			...
			if boolean == 1:
				...
				o = stump + '_AA_alinged.gb.frame.fa'
